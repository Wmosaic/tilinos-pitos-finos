...
public class JavaCap {
   ...
   /**Método para capturar un color en formato RGB
    * @param mensaje Cadena de texto que se mostrará al usuario solicitando la entrada
    * @return una cadena tipo String [rrr-ggg-bbb] que puede ser convertida a un Color que Java reconozca*/
   public String capColor(String mensaje) {
      System.out.println(mensaje);
      String aux = scanner.nextLine();

      while (!val.isColor(aux)) {
         System.out.println(mensaje + " [R, G, B]");
         aux = scanner.nextLine();
      }
      return aux;
   }
