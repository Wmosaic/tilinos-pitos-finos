...
def capturaManual() -> list:
    ...
    for _ in range(0,50,1):
        ...
        pais   = Cap.capNom("Capture el pais de procedencia: ",40)
        año    = Cap.capInte("Capture el año: ",1000)
        Escudo = Cap.capNom("Capture como se llama el Escudo: ",40)
...
def capturaFichero() -> list:
    Monedero = []
    repeticionMoneda = []
    miArchivo = Cap.capFile("Seleccione un archivo:", 
                            r'C:\Users\EnrickMC\Desktop\Ejercicios 21-40\Ejercicio 23'
                            , ".csv")
...
def main() -> None:
    AFF = "S"
    while AFF == "S" or AFF == "s":
        system("cls")
        meta()
        opc = Cap.capNom("Seleccione una opcion Manual/Fichero: ")
        match opc.lower():
            case "manual": 
                mone,rept = capturaManual()
            case "fichero":
                mone,rept = capturaFichero()
            case _: continue
        sub,tota,newDivi = calculos(mone,rept)
        salida(mone,rept,sub,tota,newDivi)
        AFF = input("Desea repetir el proceso[S/N]: ")
